...
# Set device to mps if available, else fallback to cuda or cpu
...
def prepare_data():
    ...
    encode_train = encode_train.to(device)
    encode_val = encode_val.to(device)
...
def main():
    ...
    model = Transformer(vocab_size, n_embed, block_size, n_heads, n_layers).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=lr)

    train(model, optimizer, epochs, batch_size, vocab_size, n_embed, train_data, val_data, block_size)
    
    # output = decode(model.generate(torch.zeros((1,1), dtype=torch.long, device=device), 3000).tolist()[0])
    # print(output)
